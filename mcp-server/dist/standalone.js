...
class StandaloneAlchemyMCP {
    server;
    constructor() {
        this.server = new Server({ name: 'alchemy-studio', version: '1.0.0' }, { capabilities: { tools: {} } });
    ...
    async generateImage(args) {
        ...
        if (model === 'imagen') {
            ...
            const modelInstance = ai.getGenerativeModel({ model: modelId });
    ...
    async editImage(args) {
        ...
        if (!prompt || !image_url)
            throw new Error('Missing prompt or image_url');
